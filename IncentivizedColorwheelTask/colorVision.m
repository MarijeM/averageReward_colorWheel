...
for N=1:numTrials
    ...
    if toolate==1;
        ...
        DrawFormattedText(wPtr, sprintf('First determine the correct color. Then click, and then move the mouse as fast as possible to that color.\n\nNow click to start the new trial.'),...
    ...
    elseif wheelclick == 0 
        ...
        Screen('FillOval',wPtr,pms.background,insideRect);
        Screen('TextSize',wPtr,15);
        DrawFormattedText(wPtr, sprintf('Click on the color wheel!'), 'center', 'center',[],pms.wrapAt,[],[],pms.spacing);      
        Screen('Flip',wPtr);
        GetClicks(); 
    end
