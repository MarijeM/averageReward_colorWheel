...
%% 6) add rewards to trial

% add empty .reward to trial
for i=1:pms.numBlocksPr
    for j=1:pms.numTrialsPr
        trial(j,i).reward = [];
    end
end 

% add 8 low and 8 high rewards to trial
rewards = [0.10 0.10 0.10 0.10 0.10 0.10 0.10 0.10 0.01 0.01 0.01 0.01 0.01 0.01 0.01 0.01];
Rrewards=rewards(randperm(16));

for a=1:pms.numBlocksPr
    for b=1:pms.numTrialsPr
        trial(b,a).reward=Rrewards(b) ;
    end
end

% consider preallocating for speed. dont know how to preallocate struct for
% certain size
%trial=reshape(trial,[pms.numTrials,(pms.numBlocks*2)]) ;              % reshape doubled numTrails X numBlock trial into numTrials X 2*numBlock

%if practice==1
%pms.numTrialsPr=pms.numTrialsPr*2 ;            % change number of trials for practice. change numTrialsPr(instead of numTrials)bc in showTrial,calls for numTrialsPr
%pms.numBlocksPr=pms.numBlocksPr*2 ;            % change number of blocks. change pms.numBlockPr bc in showTrial,calls for numBlocksPr
%end 
%elseif practice ==2
    % change number of trials for redo
   % pms.redoTrials=pms.redoTrials*2 ;   % gaan geen redo doen
end
