...
if level == 1
    ...
    Instruction{22} = '\n\n\n\n\n\n\n\n\n\n\n\n\n\n After the black dot, you will see an amount in euros. This is the reward that you will earn for this trial if you perform well on this trial.\nTo earn the reward, your performance needs to be at least as good as your average performance on the colorwheel during the first session. This average is computed for each condition (update or ignore) and set size separately.' ;
    imgReward=importdata('RewardcueH21.png');
    imageReward=Screen('MakeTexture',wPtr,imgReward);
    Instruction{23} = 'After the reward, the trial begins.';
    Instruction{24} = 'After every 8 trials we will tell you your total reward up to that point.'
    Instruction{25} = 'Is everything clear?\n\n It is very important that you understand this part well and we realize that it might be confusing in the beginning.\n Please contact the researchers, they will start the practice when all questions are addressed..';
...
for i=1:100 
    ...
    elseif level==4
        DrawFormattedText(wPtr,Instruction{counter},'center','center',pms.textColor,pms.wrapAt,[],[],pms.spacing);
        if mod(pms.subNo,2)==1 %every odd participant starts with the UPPS (session 2)
            DrawFormattedText(wPtr,'U',pms.xCenter+910,pms.yCenter+490,[100 100 100],pms.wrapAt,[],[],pms.spacing); 
        elseif mod(pms.subNo,2)==0 %every even participant starts with the BIS (session 2)
